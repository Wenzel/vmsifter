...
class DummyInjector(AbstractInjector):
    ...
    def get_type() -> InjectorType:
        return InjectorType.DUMMY

    def _safe_enter(self):
        super()._safe_enter()
        cmdline = self._build_proc_cmdline()
        self.logger.debug("Starting Dummy injector: %s", " ".join(cmdline))
    ...
    def _build_proc_cmdline(self) -> List[str]:
        return [
            settings.injector.dummy.injector_path,
            "--socket",
